...
### 2026-07-16 — Reversible Gone / Ruled-out toggles (+ docs debt) · Opus 4.8
> Note: the log went quiet after 2026-07-07; the 07-10 (listings a18–a25), 07-15 (income-restricted red flag, red-first ordering) and 07-16 (curation bake, Ruled-out status) sessions are recorded in `STATE.md` "Recent changes" rather than here. Resuming the log with this entry.
- **Ask:** *"for gone and rule out, user should be able to select them and unselect them, incase mistakes."*
- **Design (10-agent workflow):** mapped every status touchpoint + persistence/tests/docs, then 3 independent designs judged by 3 lenses. **"Minimal" won 2–1** (simplicity + UX over correctness). Decisive: a `prevStatus` memory field would be **silently dropped on reload for all 19 seed listings** by `mergeWithSeed`'s hand-maintained 3-field allowlist (rating/status/comments) — invisible to TypeScript — and would buy nothing on real data (the seed is 15 New + 4 Gone; no listing has ever been Shortlist/Toured). See ADR-011.
- **Built:** pure `toggleStatus(current, target)` + `HidingStatus` (components/helpers.tsx); DetailModal buttons always render, `.btn-mini.on` when lit, label flips to "Put back on list"; App.tsx collapsed both handlers into `toggleHidingStatus` and stopped closing the modal (safe — `detailApt` reads the UNFILTERED list), so undo is one click. Un-mark resets to 'New' and the toast says so.
- **Review (22-agent adversarial workflow):** 4 dimensions → 3 skeptics per finding, refute-by-default. **1 of 4 candidate findings survived** (the rest refuted as pre-existing/misattributed): removing `setDetailId(null)` made every toast **invisible** — an open `<dialog>` is in the TOP LAYER and paints over `z-index: 90`. My own DOM assertions missed it (the node exists and reads correctly; it just isn't painted).
- **Fixed:** toast → `popover="manual"` + `showPopover()` (top layer, insertion-ordered). Two gotchas found by real-browser screenshotting: `showPopover()` throws on an already-open popover (leaving it stranded below a later dialog) → `hidePopover()→showPopover()` every time; and a failed show must drop the attribute or `[popover]:not(:popover-open){display:none}` hides the toast outright. Also fixes the pre-existing Export/Settings toast occlusion.
- **Validation:** **289 tests** (+7), `tsc` + prod build clean. In-app preview: mark → modal stays open, button lights + relabels, card leaves grid, correct toast; un-mark → card returns, status New; cross-switch Gone↔Ruled out keeps exactly one lit; **un-marking seed-baked Gone a10 survives reload** (New wins over the seed). The Browser pane's screenshot has been timing out all session, so paint order was proven in **real Chrome**: a plain `z-index:999999` probe is invisible under a modal dialog while a popover probe paints above it (and popover markers at bottom:120/200 overlay the modal — the toast's own `bottom:22px` sits in the capture's cropped strip, a capture artifact, not a render bug). Final pass on unpatched code: rule out → hide → auto-hide after 2.4s → undo → restored.
- **Docs debt paid:** the Ruled-out ship had updated only README+STATE. Added **ADR-011**, an **api-contract 2026-07-16 addendum** (Status union, `showRuledOut`, `toggleStatus`, `incomeRestricted`, risk-first flag order, seed inventory 12/a6–a17 → 19/a7–a25), PRD criterion #9, tech-plan test line, README step 8. `docs/test-report.md` + `docs/review-findings.md` deliberately left as dated 2026-06-29 historical records.
