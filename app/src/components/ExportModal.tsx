...
export function ExportModal({ open, apartments, settings, sheetUrl, onClose, onToast }: Props): ReactElement {
  const [fmt, setFmt] = useState<Fmt>('tsv');

  const tsv = useMemo(() => toTSV(apartments, settings), [apartments, settings]);
  const json = useMemo(() => toJSON(apartments), [apartments]);
  ...
  async function syncSheet() {
    if (!sheetUrl) {
      onToast('Add your Google Sheet URL in Settings first');
      return;
    }
    try {
      await fetch(sheetUrl, {
        ...
        body: JSON.stringify({ rows: sheetMatrix(apartments, settings) }),
      });
      onToast('Sent — open your sheet to confirm. Empty? Re-deploy the script with access = “Anyone”.');
    } catch {
      onToast('Sync failed — check the URL in Settings');
    }
  }

  function downloadCsv() {
    const blob = new Blob([toCSV(apartments, settings)], { type: 'text/csv;charset=utf-8' });
    ...
    <Modal open={open} onClose={onClose} labelledBy="export-title">
      <div className="modal-head">
        <div>
          ...
          <div className="modal-sub">Scan your listings in a spreadsheet — or hand the data back to me.</div>
        </div>
      ...
      <div className="modal-body">
        <div className="note">
          <IconInfo />
          <span>
            <b>Sync to Google Sheet</b> pushes every listing straight into your sheet — title row + one row per
            listing, updated in place (set the URL once in <b>Settings</b>; see the README for the 5-min setup). No
            URL yet? <b>Copy</b> and paste into cell A1, or <b>Download CSV</b>. <b>JSON</b> is for pasting back to me
            so I can save your in-app edits.
          </span>
        </div>
        ...
        <div className="form-foot" style={{ gap: 8, flexWrap: 'wrap' }}>
          <button
            ...
            onClick={syncSheet}
            disabled={!sheetUrl}
            title={sheetUrl ? '' : 'Set your Google Sheet URL in Settings first'}
          >
            <IconExport />
