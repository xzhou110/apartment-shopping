...
9. **Status pipeline:** two statuses hide a listing — **Gone** (off the market entirely) and **Ruled out** (not available to you / didn't qualify; added 2026-07-16). Each has its own independent reveal chip ("Show gone" / "Show ruled out"), both default-hidden. Both are **reversible toggles** in the detail view: clicking the status a listing already has un-marks it back to **New** and returns it to the board. **Rejected** (passed by preference) stays visible unless "Hide rejected". Other statuses settable via Edit. *(Superseded the original Gone-only wording — see DECISIONS ADR-011.)*
