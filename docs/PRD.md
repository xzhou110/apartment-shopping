...
## Problem & user
- **User:** xzhou (solo). Hunting a **6–12 month lease in the Bay Area** right now.
...
- **Seed = source of truth** in `data/apartments.ts` (Claude edits it from screenshots); **localStorage**
  overlays the user's in-app state (rating + status) and remembers added/removed listings; **URL-hash share**.
...
2. **Effective monthly cost** (lightweight TCO analog): `rent + (utilities if not included) + parking + petRent`.
   A separate **move-in cost** = `deposit + appFee + brokerFee`. Both are pure derives; a **Sort → Lowest
   monthly cost** ranks by the effective figure (not just sticker rent).
3. **Lease fit** for the 6–12 month goal: fields `leaseTermMonths` / `minLeaseMonths` / `maxLeaseMonths` and a
   standing `targetMinLease`/`targetMaxLease` (default 6/12) that drive a flag when a unit can't meet the window.
4. **Apartment fields:** beds/baths/sqft, furnished, utilities-included, pet policy, available date, neighborhood/city.
5. **Tracked amenities** (tri-state pills): in-unit laundry · parking · A/C · dishwasher · pet-friendly · gym ·
   pool · balcony/patio · elevator · EV charging.
...
- **info:** unfurnished (when user targets short-term); broker fee present; far from primary anchor (> threshold, only if an anchor is set); listing stale (high `daysOnMarket` = negotiation leverage); rent well below market (verify why).
...
6. **Sort:** sort options include at least **Nearest** (distance), **Lowest monthly cost** (effective), **Rent low→high**, **Beds**, **Your rating**, **Added**. Each reorders the grid correctly.
...
8. **Compare:** ticking ≥2 listings → Compare table shows them side by side and highlights best/worst per row (cheapest rent, lowest effective monthly, nearest, most amenities, biggest sqft).
...
11. **Pure logic is tested:** distance (haversine + offline lookup), effective-monthly cost, flags, and filter/sort have Vitest unit tests that pass.
